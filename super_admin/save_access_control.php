...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($_POST as $key => $value) {
        ...
        if (strpos($key, 'access_control_') === 0) {
            ...
            $sql = "SELECT id FROM accessControl WHERE admin_id = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                $sqlUpdate = "UPDATE accessControl SET manage_user = ?, granted_date = NOW() WHERE admin_id = ?";
                $stmtUpdate = $conn->prepare($sqlUpdate);
                $stmtUpdate->bind_param("ss", $access_control_status, $admin_id);
                $stmtUpdate->execute();
            } else {
                // Insert new record if none exists
                $sqlInsert = "INSERT INTO accessControl (admin_id, manage_user, granted_date) VALUES (?, ?, NOW())";
